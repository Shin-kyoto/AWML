...
def pull_t4dataset(
    ...
    download_command = "{} data annotation-dataset pull --project-id {} --annotation-dataset-id {} --annotation-dataset-version-id {} --asset-dir {} --include-intermediate-artifacts"
...
def move_t4dataset(
    ...
    mv_command = f'mv "{from_directory}" "{to_directory}"'
    print(mv_command)
...
def download_t4dataset(
    ...
    # delete rosbag and related directories
    if delete_rosbag is True:
        for dir_name in ["input_bag", "annotation", "data"]:
            target_path = os.path.join(from_directory, dir_name)
            if os.path.exists(target_path):
                rm_command = f"rm -r {target_path}"
                print(rm_command)
                subprocess.call(rm_command, shell=True)

    # check if the dataset is successfully downloaded
    from_directory = os.path.join(from_directory, "intermediate_artifacts", "non_annotated_dataset")
    if not os.path.isdir(from_directory):
        print(f"fail to find non_annotated_dataset at {from_directory}")
        return

    # Assume only one subdirectory exists directly under from_directory
    subdirs = [d for d in os.listdir(from_directory) if os.path.isdir(os.path.join(from_directory, d))]
    assert len(subdirs) == 1, f"Expected exactly one subdirectory in {from_directory}, found {len(subdirs)}: {subdirs}"
    from_directory = os.path.join(from_directory, subdirs[0])

    # Sanitize directory name (replace ':' with '_') for the last part
    sanitized_dirname = subdirs[0].replace(':', '_')
    sanitized_to_directory = os.path.join(to_directory, sanitized_dirname)

    # Remove target directory if it already exists
    import shutil
    if os.path.exists(sanitized_to_directory):
        shutil.rmtree(sanitized_to_directory)

    move_t4dataset(from_directory, sanitized_to_directory)

    # Compress the moved directory with zstd and remove the original directory
    zstd_output = f"{sanitized_to_directory}.tar.zst"
    compress_command = f'tar -cf - -C "{os.path.dirname(sanitized_to_directory)}" "{os.path.basename(sanitized_to_directory)}" | zstd -T0 -o "{zstd_output}"'
    print(f"Compressing {sanitized_to_directory} to {zstd_output} with zstd...")
    result = subprocess.run(compress_command, shell=True)
    if result.returncode != 0:
        print(f"Compression failed for {sanitized_to_directory}")
    else:
        # Remove the original directory after successful compression
        rm_command = f'rm -rf "{sanitized_to_directory}"'
        print(f"Removing original directory: {sanitized_to_directory}")
        rm_result = subprocess.run(rm_command, shell=True)
        if rm_result.returncode != 0:
            print(f"Failed to remove {sanitized_to_directory}")
...
def main():
    ...
    for t4dataset_id, t4dataset_version_id in t4dataset_ids:
        # Get dataset info using webauto describe
        describe_command = f"{args.webauto_path} data annotation-dataset describe --project-id {args.project_id} --annotation-dataset-id {t4dataset_id} --output json"
        result = subprocess.run(describe_command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        if result.returncode != 0:
            print(f"Failed to describe dataset {t4dataset_id}: {result.stderr.decode('utf-8')}")
            continue
        try:
            info = json.loads(result.stdout.decode("utf-8"))
        except Exception as e:
            print(f"Failed to parse JSON for dataset {t4dataset_id}: {e}")
            continue
        name = info.get("name", "")
        if "shiojiri" not in name.lower():
            print(f"Skipping {t4dataset_id} because name does not contain 'Shiojiri' (case-insensitive): {name}")
            continue
